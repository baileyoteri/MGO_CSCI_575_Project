# Bailey Oteri 
# 05/04/25
# Create custom env following gym structure for SB3 for MGXS collapse project 
# Add OpenMC runs
...
class MGXS_Collapse(gym.Env): 
    ...
    def __init__(self, initial_GS_size=2000, render_mode="human"):
        ...
        self.output_path = "/media/bailey/DATA/nuclear/data/GroupStructure/nuclear-classification/openmc/Pytorch/DQN_Project/0009/"
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.ce_sp is None:  # so only loads it once per env
            self.ce_sp = openmc.StatePoint('/media/bailey/DATA/nuclear/data/GroupStructure/nuclear-classification/openmc/Pytorch/DQN_Project/Assembly/CE_to_Collapsed/output1/statepoint_ce.h5', autolink=False)
            self.ce_summary =openmc.Summary('/media/bailey/DATA/nuclear/data/GroupStructure/nuclear-classification/openmc/Pytorch/DQN_Project/Assembly/CE_to_Collapsed/output1/summary_ce.h5')
            self.ce_sp.link_with_summary(self.ce_summary)

            self.ce_keff = self.ce_sp.keff.nominal_value
            self.initial_mg_sp = openmc.StatePoint('/media/bailey/DATA/nuclear/data/GroupStructure/nuclear-classification/openmc/Pytorch/DQN_Project/Assembly/CE_to_Collapsed/output1/statepoint_mg.h5', autolink=False)
